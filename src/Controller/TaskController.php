...
final class TaskController {
	...
	const TABLE_TASKS = PMS_TABLE_PREFIX . 'tasks';
	const TABLE_TASK_MEMBERS = PMS_TABLE_PREFIX . 'task_members';
	const TABLE_COMMENTS = PMS_TABLE_PREFIX . 'comments';

	const TABLE_ACTIVITY_LOG = PMS_TABLE_PREFIX . 'activity_log';

	const TABLE_ATTACHMENTS = PMS_TABLE_PREFIX . 'attachments';
	...
	public function create(WP_REST_Request $request){
		...
		$slug = SlugGenerator::slug($name, self::TABLE_TASKS, 'slug');
		...
		if($tags && sizeof($tags)>0){
			$taskTagsTable = PMS_TABLE_PREFIX . 'task_tags';

			$tagObj = new TagController();
	...
	public function update(WP_REST_Request $request){
		...
		if(sizeof($members)>0){
			$taskMembersTable = PMS_TABLE_PREFIX . 'task_members';
		...
		if( sizeof($properties) > 0 ) {
			...
			$activitiesLogTable = PMS_TABLE_PREFIX . 'activity_log';
	...
	public function updateTaskSortOrder(WP_REST_Request $request){
		...
		$tableTasks = PMS_TABLE_PREFIX . 'tasks';
...
FROM {$wpdb->prefix}pms_tasks as tasks
				...
				foreach ($childResults as $value) {
					...
					if($value['assignedToId']){
						$assignedTo = [
							...
							'avatar' => UserController::getUserAvatar($value['assignedToId']),
						];
					}
					$priority = null;
					if($value['priorityId']){
			...
			foreach ( $parentResults as $key => $result ) {
				...
				if($result['assignedToId']){
					$assignedTo = [
						...
						'avatar' => UserController::getUserAvatar($result['assignedToId']),
					];
				}
				$priority = null;
	...
	public function getTaskSectionsByProjectId($projectId){
		global $wpdb;
		$db = DatabaseTableSchema::get_global_wp_db($wpdb);
		$taskSectionsTable = PMS_TABLE_PREFIX . 'task_sections';
	...
	public function getParentTaskBySlug($slug, $projectId) {
		...
		$tableTask = PMS_TABLE_PREFIX . 'tasks';
		$tableProjects = PMS_TABLE_PREFIX . 'projects';
	...
	public function createTaskSection(WP_REST_Request $request){
		...
		$tableTaskSection = PMS_TABLE_PREFIX . 'task_sections';

		$slug = SlugGenerator::slug($name, $tableTaskSection, 'slug' );
	...
	public function createComment(WP_REST_Request $request){
		...
		// Start a transaction
		$db->query('START TRANSACTION');
	 $commentsTable = PMS_TABLE_PREFIX . 'comments';
		// Insert the task into the database
...
//			$row['children'] = $this->getCommentsByParentId($row['id']);
	...
	public function getActivityLogsByTaskId($subjectId, $subjectName){
		...
		global $wpdb;
		$db = DatabaseTableSchema::get_global_wp_db($wpdb);
		$activityLogTable = PMS_TABLE_PREFIX . 'activity_log';
		$usersTable = $wpdb->prefix . 'users';
...
//		return new WP_REST_Response(['status'=>200, 'message'=>'Attachment remove successfully', 'data'=>$taskId, 'loggedUserID'=>$userId ], 200);
	...
	public function getAttachmentsByTaskId($taskId, $subjectName = 'task'){
		...
		global $wpdb;
		$db = DatabaseTableSchema::get_global_wp_db($wpdb);
		$attachmentsTable = PMS_TABLE_PREFIX . 'attachments';
		$usersTable = $wpdb->prefix . 'users';
...
FROM {$wpdb->prefix}pms_tasks as tasks
	...
	public function getQuickTaskById( $id ) {
		...
		$tasksTable = PMS_TABLE_PREFIX . 'quick_tasks';
...
//		return new WP_REST_Response(['status'=>200, 'message'=>'Quick task created successfully', 'data'=>$requestData ], 200);
		...
		$tableName = PMS_TABLE_PREFIX . 'quick_tasks';
